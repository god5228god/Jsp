...
<%
	...
	try
	{
		...
		if(op.equals("+")){
			...
			result = String.format("%d %s %d = %.1f ", num1, op, num2, num1/(double)num2);	
		} 
	...
	// check
	RequestDispatcher dispatcher =  request.getRequestDispatcher("Receive15.jsp");
	...
	※ 처리 과정 및 개념
		...
		일반적으로 HttpServlet을 상속받는 클래스 .. 서블릿(Servlet)
	
		이렇게 작성된 클래스 내부에는 
		실제 요청을 서비스하는 『doGet()』과 『doPost()』 메소드가
		정의되어 있으며 (→ 『service()』 메소드가 이들의 상위 메소드)
	
		ServletContainer는 『HttpServle』t의 인스턴스를 생성하고,
		『init()』 메소드를 호출하여 실행시켜주고,
		이 메소드에 의해 매핑된 URL에
		(페이지 요청 방식에 따라) doGet(), doPost() 중
		선택하여 메소드를 호출하여 실행시켜주고,
		...
		멀티 스레드에 안전하게 설계(스레드 세이프티)되어야 하고
		따라서, 멀티 스레드에 안전하게 설계하지 않은 경우
		상위 클래스를 마구 접근하게 되어 에러가 발생할 수 밖에 없다.
	
		이와 같은 이유로 환경 설정이나 J2EE 서비스에 관한 내용은
		『ServletContext』에서 할 수 있게 된다.
		(※ ServeltContext: 서블릿에 대한 환경, 상태 등을 설정할 수 있는 객체)
	
		이 『ServeltContext』는 『getServletContext()』 메소드로만 받을 수 있다.
		...
		이 일은 Dispatcher가 대신 수행해 준다는 것이다.
		하지만, 이 Dispatcher는
		HttpServletRequest, HttpServletResponse를
		생성해줄 수는 없다.
		그렇기 때문에 Dispatcher가 생성해 준 새로운 서블릿 스레드를
