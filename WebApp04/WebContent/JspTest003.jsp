...
<!-- 
	...
	브라우저에서 새로고침을 반복해보면 (요청 반복)
	
	a 변수(선언부에서 선언한 변수)값만 계속 증가하는 것을 확인할 수 있다.
	a 변수의 값만 계속 증가하는 이유를 확인해 보기 위해서는
	...
	==> 변수 a는 클래스(JspTest003_jsp)의 전역변수로 선언된 것을 확인할 수 있다.
		...
		변수 b는 메소드(JspTest003_jsp 클래스의 _jspService())의 지역변수로
		선언된 것을 확인할 수 있다.
		
		『_jspInit()』메소드와 『_jspDestroy()』메소드는
		 서비스가 시작될 때와 끝날 때 각각 한 번 씩만 호출되는(실행되는) 메소드이다.
		 
		 반면, 『_jspService()』메소드는
		 브라우저에 의해 페이지가 요청될 때 마다
		 매번 호출되는(실행되는) 메소드이다.
		 
		 이로 인해 변수 b는 페이지에 요청이 발생할 때마다
